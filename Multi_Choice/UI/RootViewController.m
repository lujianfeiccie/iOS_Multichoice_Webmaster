...
- (void)viewDidLoad
{
    ...
    for (NSInteger i=1; i<=9; i++)
    {
        ...
        model.m_text = [NSString stringWithFormat:@"选择题第%li卷",(long)i];
        model.m_value = [NSString stringWithFormat:@"2014_part%li",(long)i];
        model.m_type = TYPE_MULTI_CHOICE;
        [m_datalist addObject:model];
    }
    model = [[ModelData alloc]init];
    model.m_text = @"简答题";
    model.m_value = @"2014_short_answer";
...
-(void) viewDidLayoutSubviews
{
  ...
  //  NSLog(@"viewDidLayoutSubviews");
    [m_tableview_list setFrame:CGRectMake(0,0,
                                          [UIScreen mainScreen].applicationFrame.size.width,
                                          [UIScreen mainScreen].applicationFrame.size.height-self.navigationController.navigationBar.frame.size.height)];
...
-(void) threadSearch :(NSString*) keywords
{
    ...
    for (NSInteger i=1; i<=9; i++) {
        [mutli_question_filename addObject:[NSString stringWithFormat:@"2014_part%li",(long)i]];
    }
